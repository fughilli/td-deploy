...
semantics natively.** TD is removed from the _deployment_ path and kept only as:
...
The importer is ~20% of the work. The other 80% is re-implementing a _subset_ of TD's operator
...
| Import path       | **`toeexpand`**, _not_ TDXN | TDXN requires a live TD runtime via MCP and drops GLSL/feedback detail. `toeexpand` is a standalone file transform. |
...
next frame. The buffers on those edges _are_ the state vector — allocated once, ping-ponged each
...
- **I/O as graph-declared services (phase 2):** an I/O operator's presence _declares_ a runtime
...
within tolerance. This is the _only_ safe way to grow operator coverage, and the legitimate
...
- **M0 — Verify-first (gating).** (a) `toeexpand` runs headless on Linux w/o license/GPU;
  ...
  _Nothing downstream is designed until both are green._
- **M0.5 — MLIR skeleton.** Out-of-tree `tox` dialect (TableGen types+ops) building against
  ...
  toolchain before the full slice. _Fallback if this drags: a thin interpreter behind the same
  IR so first-pixels isn't blocked on MLIR._
...
7. **Pi perf** — targets projects that _fit_ the budget; TOP-heavy 2D is the sweet spot.
