...
class ChopToToxTest(unittest.TestCase):
    def test_ascii_dag(self):
        ...
        chops = [
            ...
            {"name": "speed1", "type": "speed", "inputs": ["constant1"],
             "channels": []},
        ]
        ir = emit(chops)
        ...
        self.assertIn(
            ...
            "(!tox.chop<16>) -> !tox.chop<1>", ir)
        # Speed integrates its input, same width.
        self.assertIn(
            "%speed1 = tox.chop_speed %constant1 : "
            "(!tox.chop<1>) -> !tox.chop<1>", ir)

    def test_literal_constant(self):
        ir = emit([{"name": "c", "type": "constant", "inputs": [],
                    "channels": ["0.5", "-1", "2.0"]}])
    ...
    def test_null_is_passthrough(self):
        ...
        self.assertIn("%n = tox.chop_select %c : "
                      "(!tox.chop<1>) -> !tox.chop<1>", ir)
